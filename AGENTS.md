...
- **按需动态加载**：
  ...
  - Excalifont 是默认字体，必须保留在 `packages/excalidraw/fonts/Excalifont` 本地加载；其他字体优先使用版本化 CDN 样式表或远程字体文件。CDN 样式表内部注册分片 `@font-face` 时，编辑器必须使用样式表声明的 CSS 字体名，不得把 CSS URL 写入 `@font-face src`，并同步在 CSP 中放行 `cdn.jsdelivr.net` 的样式与字体来源；
