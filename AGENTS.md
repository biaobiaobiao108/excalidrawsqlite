...
## 3. Prettier 格式规范 (Mandatory)

- 所有受项目 Prettier 配置管理的文件必须严格遵守 Prettier 格式，不得手工保留与格式化结果不一致的写法。
- 每次提交前必须运行 `bun run test:other`，并确保命令以退出码 0 完成；发现差异后先格式化再重新检查。
- 格式化整个仓库使用 `bun run fix:other`；只格式化指定文件时使用本地 Prettier CLI（如 `bunx prettier --write <文件路径>`），不要向根目录的 `bun prettier` 脚本追加文件参数，以免意外触发全仓库格式化。
- 不得通过修改 Prettier 配置、忽略文件或降低检查范围来绕过格式错误，除非用户明确要求。
