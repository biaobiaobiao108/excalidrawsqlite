...
- **增量空间自动收缩**：配置 SQLite `PRAGMA auto_vacuum = INCREMENTAL`，在清空回收站、永久删除画板与定时维护任务中平滑收缩 `.db` 文件，将空闲页归还给操作系统文件系统。
...
- **回收站与生命周期保护**：支持画板软删除至回收站、一键还原与彻底清空；支持可配置的超期自动清理机制（默认 30 天 TTL），超期自动物理删除并级联触发孤立附件 GC。
